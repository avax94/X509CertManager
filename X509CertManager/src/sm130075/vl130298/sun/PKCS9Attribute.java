...
/**
 * Class supporting any PKCS9 attributes. Supports DER decoding/encoding and
 * access to attribute values.
 *
 * <a name="classTable">
 * <h3>Type/Class Table</h3></a> The following table shows the correspondence
 * between PKCS9 attribute types and value component classes. For types not
 * listed here, its name is the OID in string form, its value is a
 * (single-valued) byte array that is the SET's encoding.
 *
 * <P>
 ...
 * 
 * <TR>
 * <TD>1.2.840.113549.1.9.19</TD>
 * <TD>Algorithm id</TD>
 * <TD>Single-valued</TD>
 * <TD>CertificateAlgorithmId</TD>
 * </TR>
 * 
 * <TR>
 * <TD>1.2.840.113549.1.9.20</TD>
 * <TD>Validity</TD>
 * <TD>Single-valued</TD>
...
public class PKCS9Attribute implements DerEncoder {
	 ...
	 */
	static final ObjectIdentifier[] PKCS9_OIDS = new ObjectIdentifier[22];

	private final static Class<?> BYTE_ARRAY_CLASS;

	static { // static initializer for PKCS9_OIDS
		for (int i = 1; i < PKCS9_OIDS.length - 6; i++) {
		...
		PKCS9_OIDS[PKCS9_OIDS.length - 6] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 16, 2, 12 });
		PKCS9_OIDS[PKCS9_OIDS.length - 5] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 16, 2, 14 });
		PKCS9_OIDS[PKCS9_OIDS.length - 4] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 17 });
		PKCS9_OIDS[PKCS9_OIDS.length - 3] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 18 });
		PKCS9_OIDS[PKCS9_OIDS.length - 2] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 19 });
		PKCS9_OIDS[PKCS9_OIDS.length - 1] = ObjectIdentifier
				.newInternal(new int[] { 1, 2, 840, 113549, 1, 9, 20 });
	...
	public static final ObjectIdentifier SERIAL_OID = PKCS9_OIDS[18];
	public static final ObjectIdentifier CERT_VERSION_OID = PKCS9_OIDS[19];
	public static final ObjectIdentifier ALGORITHM_ID_OID = PKCS9_OIDS[20];
	public static final ObjectIdentifier VALIDITY_OID = PKCS9_OIDS[21];
	...
	public static final String SERIAL_NUMBER_STR = "CertificateSerialNumber";
	public static final String VERSION_STR = "CertificateVersion";
	public static final String ALGORITHM_ID_STR = "CertificateAlgorithmId";
	public static final String CERTIFICATE_VALIDITY_STR = "CertificateValidity";
	...
	static { // static initializer for PCKS9_NAMES
		...
		NAME_OID_TABLE.put("certificateserial", PKCS9_OIDS[18]);
		NAME_OID_TABLE.put("certificateversion", PKCS9_OIDS[19]);
		NAME_OID_TABLE.put("certificateAlgorithmId", PKCS9_OIDS[20]);
		NAME_OID_TABLE.put("certificatevalidity", PKCS9_OIDS[21]);
	};
	...
	static {
		...
		OID_NAME_TABLE.put(PKCS9_OIDS[18], SERIAL_NUMBER_STR);
		OID_NAME_TABLE.put(PKCS9_OIDS[19], VERSION_STR);
		OID_NAME_TABLE.put(PKCS9_OIDS[20], ALGORITHM_ID_STR);
		OID_NAME_TABLE.put(PKCS9_OIDS[21], CERTIFICATE_VALIDITY_STR);
	}
	...
	private static final Byte[][] PKCS9_VALUE_TAGS = { null, { new Byte(DerValue.tag_IA5String) }, // EMailAddress
			...
			{ new Byte(DerValue.tag_Integer) }, //SerialNumber
			{ new Byte(DerValue.tag_Integer) }, //Version
			{ new Byte(DerValue.tag_Sequence) }, //Algorithm id
			{ new Byte(DerValue.tag_Sequence) } //Certificate validity
	};

	private static final Class[] VALUE_CLASSES = new Class[22];
	...
	static {
		try {
			...
			VALUE_CLASSES[18] = // CertificateSerialNumber
					Class.forName("sun.security.x509.CertificateSerialNumber");
			VALUE_CLASSES[19] = // CertificateVersion
					Class.forName("java.lang.Integer");
			VALUE_CLASSES[20] = // CertificateAlgorithmId
					Class.forName("sun.security.x509.CertificateAlgorithmId");
			VALUE_CLASSES[21] = // CertificateValidity
	...
	/**
	 * The index of the OID of this attribute in <code>PKCS9_OIDS</code>, or -1
	 * if it's unknown.
	 */
	private int index;
	 ...
	 * <code>VALUE_CLASSES[index]</code>. The SET itself as byte[] if unknown.
	 */
	...
	public PKCS9Attribute(DerValue derVal) throws IOException {
		...
		case 17: // SignatureTimestampToken attribute
			...
			break;
		case 18: // CertificateSerialNumber attribute
			value = new CertificateSerialNumber(elems[0]);
			break;
		case 19: // CertificateVersion
			value = elems[0].getInteger();
			break;
		case 20: // CertificateAlgorithmId
			value = new CertificateAlgorithmId(AlgorithmId.parse(elems[0]));
			break;
		case 21: // CertificateValidity
			DerInputStream derInput = new DerInputStream(elems[0].toByteArray());
			value = new CertificateValidity(derInput);
			break;
	...
	public void derEncode(OutputStream out) throws IOException {
		...
		case 18: // SerialNum
			...
			CertificateSerialNumber csn = (CertificateSerialNumber)value;
			csn.encode(tmp2);
			temp.write(DerValue.tag_Set, tmp2);
			break;
		case 19: // Cert version
			tmp2 = new DerOutputStream();
			tmp2.putInteger((int) value);
			temp.write(DerValue.tag_Set, tmp2.toByteArray());
			break;
		case 20: // CertificateAlgorithmId
			tmp2 = new DerOutputStream();
			CertificateAlgorithmId algId = (CertificateAlgorithmId)value;
			algId.encode(tmp2);
			temp.write(DerValue.tag_Set, tmp2.toByteArray());
			break;
		case 21: // CertificateValidity
			tmp2 = new DerOutputStream();
			CertificateValidity cVal = (CertificateValidity)value;
			cVal.encode(tmp2);
			temp.write(DerValue.tag_Set, tmp2.toByteArray());
			break;
	...
	/**
	 * Returns if the attribute is known. Unknown attributes can be created from
	 * DER encoding with unknown OIDs.
	 */
	 ...
	 * attribute whose value it is. This method will only be called for known
	 * tags.
	 */
